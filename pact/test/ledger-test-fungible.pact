(module ledger-test-fungible G
  " Module for mocking an individual babena token \
  ...
  (defun install (cap:object)
    (install-capability
      (free.demo-ledger.TRANSFER TOKEN
        (at 'sender cap)
        (at 'receiver cap)
        (at 'amount cap)
      ))
  ...
  (defun transfer:string
    ...
    (with-capability (TRANSFER sender receiver amount)
      ...
      (with-capability
        ...
        (free.demo-ledger.transfer TOKEN sender receiver amount))
    )
      ...
      (with-default-read entries receiver
        ...
        (let
          ...
          (with-capability (DELEGATE sender (at 'version (read entries sender)))
            (free.demo-ledger.transfer-create
  ...
  (defun get-balance:decimal
    ( account:string )
    (free.demo-ledger.get-balance TOKEN account)
  )
  ...
  (defun details:object{fungible-v2.account-details}
    ...
    (remove "id"
      (+ { 'guard: (at 'guard (read entries account))}
        (free.demo-ledger.details TOKEN account)))
  )

  (defun precision:integer ()
    (free.demo-ledger.precision TOKEN)
  )

  (defun enforce-unit:bool
    ( amount:decimal )
    (free.demo-ledger.enforce-unit TOKEN amount)
  )
  ...
  (defun create-account:string
    ...
    (free.demo-ledger.create-account TOKEN account (delegate-guard account 0))
  )
  ...
  (defun rotate:string
    ...
    (let*
      ...
      (with-capability (DELEGATE account v)
        (free.demo-ledger.rotate TOKEN account
  ...
  (defun test-fund
    ...
    (free.demo-ledger.credit TOKEN account (delegate-guard account 0) amount)
    true
  )
