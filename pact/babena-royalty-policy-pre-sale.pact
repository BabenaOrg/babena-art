...
(module babena-royalty-policy GOVERNANCE
  ...
  (defun enforce-reserved (account:string)
    (enforce (contains account (get-reservations)) "You did not reserve any tokens in the pre-sale")
  )
    ...
    ( token:object{token-info}
     ...
     (with-capability (MINT account)
       ...
       (cond
         ...
         ((contains account (get-accounts))
           [true (update-status token-id account available-tokens current-length minted)]
         )
        (whitelist-enabled
             [(enforce-whitelist account guard)
             (let  ( (claimed:integer (at 'claimed (get-whitelist-info account)))
                     (price:decimal (at 'whitelist-price babena-details)))
                     (coin.transfer account creator price)
                     (update whitelists account{
                      "claimed": (+ claimed 1)
                      })
                     (update-status token-id account available-tokens current-length minted)
              )])
              [(coin.transfer account creator (at 'price-per-nft babena-details))
              (update-status token-id account available-tokens current-length minted)
              ]
           )
        )
  ...
  (defun update-status (token-id:string account:string available-tokens:[string] current-length:integer minted:integer)
  ...
  (defun get-reservations ()
    (babena-pre-sale.get-accounts)
  )
